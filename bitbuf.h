...
/* Set all values to 0 and reset the length while maintaining originally allocated memory
 ...
 * You now own the storage the bit occupies and is your responsibility to free() it
 */
 ...
 * Used when you typically know the size of data and want to avoid repetitive realloc()s
 * Should not be used often but can be useful for performance
 */
void bitbuf_grow( bitbuf *, size_t );

/* Set the length of buffer
 * This does *NOT* allocate new memory and should not perform a `bitbuf_setlen()` to a 
 ...
 */
int bitbuf_find( const bitbuf *src, const bitbuf *pat, size_t garble, size_t offset );

/* Replace the first occurence of `old` with `fresh`
 * Returns the number of patterns replaced
 */
int bitbuf_replace( bitbuf *src, const bitbuf *old, const bitbuf *fresh, size_t garble, size_t start, size_t end );

/* Compare two buffers. Return an integer less than, equal to or greater than zero if the first
 * buffer is found to be respectively less than, equal to or greater than the second buffer
 */
...
/* Zero is added as padding to the shorter buffer to match the length of the longer one */
