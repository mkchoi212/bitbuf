...
int bitbuf_replace( bitbuf *src, const bitbuf *old, const bitbuf *fresh, size_t garble, size_t start, size_t end ) {
	...
	if( old->len == 0 )
		die( "replace: Cannot replace an empty buffer" );
...
int bitbuf_cmp( const bitbuf *a, const bitbuf *b ) {
	...
	if( a->len != b->len )
		die( "cmp: Buffers should be the same length" );
...
unsigned char bitbuf_getbyte( const bitbuf *bb, size_t pos, size_t offset ) {
	...
	if( ( pos + 1 ) * 8 + offset > bb->len )
		die( "getbyte: Out of bounds" );
...
void bitbuf_op( const bitbuf *a, const bitbuf *b, bitbuf *res, OperatorPtr op ) {
	...
	if( res->alloc <= a->len )
		bitbuf_grow( res, a->len - res->alloc + 8 );
